...
const DRDOJobs = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-gray-900 to-black text-white p-8 flex flex-col items-center">
      ...
      >
        <h1 className="text-6xl font-extrabold mb-6">DRDO Job Listings</h1>
        <input
          type="text"
          placeholder="Search for a job..."
          className="w-full p-4 text-gray-800 rounded-lg border-2 border-gray-300 focus:ring-4 focus:ring-blue-400 focus:border-transparent"
          value={searchQuery}
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredJobs.map((job) => (
          <motion.div
            key={job.id}
            className="bg-white text-gray-800 rounded-xl shadow-xl p-6 cursor-pointer hover:shadow-2xl transition-transform transform hover:scale-105"
